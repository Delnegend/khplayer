...
const KHPGen = {
  ...
  REMOVE_EPISODE(elem) {
    const a = $(elem.parentNode.parentNode);
  ...
  generateCodeFromUserInput(addPosterAndPlayingIndicator = true) {
    ...
    // Xử lý từng episode
    for (const value of document.querySelectorAll('#userInteractCtn .episode_container')) {
      if (value.querySelector('.title').value) {
        // Object tạm thời để đẩy dần dữ liệu vào, cuối cùng đẩy vào RAW_DATA
        const processingEpElem = {};
        ...
        const preThumbSrc = value.querySelector('.previewThumbnail').value;
        if (preThumbSrc) {
          processingEpElem.previewThumbnails = {
            src: preThumbSrc,
          };
        }

        // Xử lý từng source
        for (const source of value.querySelectorAll('#ALL_SRCS_CTN .singleSrcCtn')) {
        ...
        // Xử lý từng phụ đề
        for (const caption of value.querySelectorAll('#ALL_CAPS_CTN .singleCapCtn')) {
          if (caption.querySelector('.captionFullName').value) {
            const processingCaption = {};
  ...
  messageBox(message = 'Thông báo!', duration = 1000) {
    ...
    const tempID = KHPGen.randomString();
...
const UNIQUE_ID = 'khplayer' + KHPGen.randomString();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (language == 'en') {
    ...
    for (const elem of document.querySelectorAll('*[ph-en]')) elem.setAttribute('placeholder', elem.getAttribute('ph-en'));
  } else {
    for (const elem of document.querySelectorAll('i-18n')) elem.innerHTML = elem.getAttribute('vi');
    for (const elem of document.querySelectorAll('*[ph-vi]')) elem.setAttribute('placeholder', elem.getAttribute('ph-vi'));
  }
});
